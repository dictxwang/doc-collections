...
git reset	# 将添加到暂存区的未提交的变更取回到工作区（工作区变更会保留）

git reset --hard HEAD  # 回退到最新版本（工作区变更不会保留）

git reset --hard CommitId  # 回退到指定的版本（工作区变更不会保留）
...
git log [-p] master..dev	# 对比dev相比master有哪些不同的提交（不包含master有dev无）

git log [-p] master...dev	# 对比master和dev从拉出分支共有哪些不同的提交（包含master有dev无）
